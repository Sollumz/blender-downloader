...
DATA_DIR = user_data_dir(
    appname=__title__, appauthor=__author__, version=__version__)
...
BLENDER_RELEASE_DOWNLOADS_URL = 'https://download.blender.org/release/'
...
def get_toplevel_dirnames_from_paths(paths):
    """Get the names of the directories in the top level from a set of paths.
...
def clean_other_versions_cache():
    """Remove cache from other possible installed versions of the program.
...
def build_parser():
    ...
    parser.add_argument(
        ...
        " (latest development release). The minimum version supported is"
        f" {MINIMUM_BLENDER_VERSION_SUPPPORTED}.",
    )
    parser.add_argument(
        '-d',
        '--output-directory',
    ...
    parser.add_argument(
        ...
        ' versions. You can pass an optional maximum number of versions to'
        ' list.',
    )
    parser.add_argument(
        '-n',
...
def parse_args(args):
    ...
    if callable(opts.operative_system):
    ...
    if callable(opts.output_directory):
    ...
    if opts.remove_compressed and not opts.extract:
        sys.stderr.write(
            "The option '--remove-compressed' only makes sense passed"
            " along with the option '--extract'.\n",
        )
        sys.exit(1)
    ...
    if (
        opts.bits == 32 and
        BlenderVersion(opts.blender_version) > BlenderVersion('2.80')
    ):
        sys.stderr.write(
...
def guess_stable_version_number_from_daily_builds_page(use_cache=True):
    """Try to get the stable Blender version from dayly builds page.

    See https://builder.blender.org/download/daily/

    This method has been unstable in the past so is recommended to
    not be used as the unique reliable source.
    ...
    stable_Version = None
    for line in (line.lower() for line in res.split('<')):
        if 'stable' in line:
    ...
    if identifier == 'stable':
        ...
        if latest_Version is None:
            ...
            for minor_version, version_data in versions_json.items():
                if 'dev' in version_data:  # (develop)
                    continue
    ...
    if operative_system == 'windows':
        ...
        def valid_release_file(filename):
            ...
            if major_minor_blender_Version > BlenderVersion('2.80'):
                ...
                bits_id = 'x86_64' if (
                    bits == 64 or arch == 'x86_64') else 'i686'
        ...
        def valid_release_file(filename):
            if major_minor_blender_Version < BlenderVersion('2.72'):
                ...
                bits_id = 'x86_64' if (
                    bits == 64 or arch == 'x86_64') else 'i386'
                if not filename.endswith(f'{bits_id}{compressed_ext}'):
                    return False
                ...
                if arch in {'x64', 'arm64'}:
                    ...
                    return False
            elif not filename.endswith(compressed_ext):
                return False
            return True
...
def get_legacy_release_download_url(
    ...
    res = GET(BLENDER_RELEASE_DOWNLOADS_URL, use_cache=use_cache)
    ...
    url = f'{BLENDER_RELEASE_DOWNLOADS_URL}{version_path}'
    ...
    try:
        ...
        if os.path.isfile(output_filepath):
            sys.stderr.write(
                ...
                " remove the file before execute blender-downloader.\n",
            )
            sys.exit(1)
        ...
        if short_extension == '.zip':
            ...
            with zipfile.ZipFile(zipped_filepath, 'r') as f:
                ...
                if len(root_dirnames) > 1 or not root_dirnames:
                    ...
                    if (
                        ...
                        sys.stderr.write(
                            f"The directory '{output_directory}' where the"
                            " files will be extracted already exists and is"
                            " not empty. Extraction skipped.\n",
                        )
                        sys.exit(1)
            ...
            with tarfile.open(zipped_filepath, 'r') as f:
                ...
                if len(root_dirnames) > 1 or not root_dirnames:
                    ...
                    extracted_directory_path = os.path.join(
                        output_directory, removesuffix(
                            zipped_filename, long_extension),
                    )
...
def print_executables(
    ...
    """Print Blender and Blender's Python executable paths to STDOUT.
    ...
    if operative_system == 'linux':
        ...
        for dirpath, _, files in os.walk(extracted_directory_path):
    ...
    if print_python_executable:
        if python_executable_filepath is None or not os.path.isfile(
            ...
            sys.stderr.write(
                'Builtin Blender Python interpreter executable'
                ' path not found\n',
            )
            error = True
        else:
...
def list_available_blender_versions(
    ...
    The printing order is from greater versions to lower. You can specify
    a maximum number of versions to print using the ``maximum_versions``
    argument.
    ...
    response_lines = GET(BLENDER_RELEASE_DOWNLOADS_URL,
                         use_cache=use_cache).splitlines()
    version_matcher = re.compile(r'^\d+\.\d+$')
    min_blender_Version_supported = BlenderVersion(
        MINIMUM_BLENDER_VERSION_SUPPPORTED)
    ...
    for line in reversed(response_lines):
        ...
        url = f'{BLENDER_RELEASE_DOWNLOADS_URL}Blender{blender_version}/'
        repo_response = GET(url, use_cache=use_cache).splitlines()
        ...
        for repo_line in reversed(repo_response):
            ...
            if not repo_line.split('-', 2)[2].startswith(
                expected_os_identifier,
            ):
                continue
        ...
        for version_ in repo_versions:
            # found version
            if version_.count('.') > 2:
                version = '.'.join(version_.split('.')[:3])
            else:
                version = version_
            ...
            if (
                not _stable_version_printed and
                BlenderVersion(version) < stable_Version
            ):
                sys.stdout.write(f'{stable_version} (stable)\n')
...
def run(args):  # noqa: PLR0911
    ...
    try:
        ...
        try:
            ...
            sys.stderr.write(
                f"The release '{opts.blender_version}' can't be"
                " located in official Blender repositories.\nMake sure"
                " that you are passing a valid version.\n\n"
                f"If you think that '{opts.blender_version}' is a valid"
                " release and this is a problem with blender-downloader,"
                f" please, report it to {SCRIPT_NEW_ISSUE_URL}\n",
            )
            return 1
    except KeyboardInterrupt:
        return 1
    ...
    try:
        ...
        if opts.remove_compressed and os.path.isfile(
            downloaded_release_filepath,
        ):
            os.remove(downloaded_release_filepath)
